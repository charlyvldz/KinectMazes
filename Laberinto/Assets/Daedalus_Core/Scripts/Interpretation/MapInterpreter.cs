...
//		BuildObject(map,cell.location,storey,cell.Type,cell.Orientation);
		//		foreach(VirtualCell.CellType t in cell.GetSubTypes()) BuildObject(map,cell.location,storey,t,cell.Orientation);
	...
	public void ConvertDoor(VirtualMap map, VirtualCell conversion_cell){
		if(conversion_cell.IsDoor()) {
			if(!behaviour.drawDoors) conversion_cell.Type = VirtualCell.CellType.EmptyPassage;
				...
				CellLocation prev_loc = map.GetNeighbourCell(conversion_cell.location,conversion_cell.Orientation);
				CellLocation next_loc = map.GetNeighbourCell(conversion_cell.location,map.GetDirectionOpposite(conversion_cell.Orientation));
//				Debug.Log (prev_loc);
//				Debug.Log (next_loc);
...
//				Debug.Log (map.GetCell(next_loc).IsRoomFloor());
	...
	// This will convert 'Ninguno' cells to columns where necessary.
	protected bool ConvertColumn(VirtualMap map, VirtualCell conversion_cell, bool mayBeDirectional = true){
		...
		if (map.IsRemovable(l,behaviour.drawWallCorners)){
			...
			foreach(VirtualMap.DirectionType dir in map.directions){
				CellLocation neigh_loc = map.GetNeighbourCell(l,dir);
...
//				if (behaviour.createColumnsInRooms){
					// We at last check whether that column is actually in a room, or in the corridor
					if (map.IsInRoom(l)){
						if (behaviour.createColumnsInRooms) conversion_cell.Type = VirtualCell.CellType.InsideRoomColumn;
						else{
							conversion_cell.Type = VirtualCell.CellType.None;
							createColumn = false;
						}
					}
					else if (map.IsOnRoomBorder(l)) conversion_cell.Type = VirtualCell.CellType.RoomColumn;
					else conversion_cell.Type = VirtualCell.CellType.CorridorColumn;	// Rare case of a corridor column alone in the corridor
//				} else {
//					createColumn = false;
//				}
			} 
		...
		if (behaviour.useDirectionalFloors){
			...
			for(int i=0; i<border_neighs.Length; i++){
				...
				if (!map.LocationIsOutsideBounds(other_l) // && other_l.isValid()
					...
					if (other_cell.IsWall() && !map.IsRemovable(other_cell.location)){
						countWallNeighs++; 
						validIndices[i] = true;
					}
				}
	...
	public void ConvertDirectionalRoomColumn(VirtualMap map, VirtualCell conversion_cell){
		...
		if (behaviour.useDirectionalFloors){
			...
			for(int i=0; i<border_neighs.Length; i++){
				CellLocation other_l = border_neighs[i];
				if (!map.LocationIsOutsideBounds(other_l) // && other_l.isValid() 
				    && ((map.GetCell(other_l).IsRoomWall())
				    || (considerDoorsAsWalls && map.GetCell(other_l).IsDoor()))
				    ){
					countWallNeighs++; 
					validIndices[i] = true;
...
//			Debug.Log ("Cell " + l + " has neigh walls " + countWallNeighs);
	...
	protected void ConvertWallOrientation(VirtualMap map, VirtualCell conversion_cell){
		...
		CellLocation neigh_loc = map.GetNeighbourCell(conversion_cell.location,VirtualMap.DirectionType.North);
		if (!map.LocationIsOutsideBounds(neigh_loc)){
			VirtualCell neigh_cell = map.GetCell(neigh_loc);
...
//		Debug.Log (conversion_cell.Orientation);
	...
	protected bool CheckWallRendering(VirtualMap map, VirtualCell conversion_cell){
		...
		// TODO: Probably IsRemovable does this for us!!!
		
		// We check the neighbours. A rendered wall needs at least one floor nearby.
		CellLocation[] floor_neighs = map.GetAllNeighbours(conversion_cell.location);
		int countFloorNeighs = 0;
		...
		if (countFloorNeighs > 0){// || conversion_cell.Type == VirtualCell.CellType.PerimeterWall){	// We always draw the perimeter!
...
//		Debug.Log ("CHECKING STAIRS FOR STOREY " + storey);
		...
		foreach(CellLocation l in currentMap.roomCells){
			...
			foreach(VirtualMap.DirectionType direction in currentMap.directions){
				CellLocation next_l = currentMap.GetNeighbourCellAtStep(l,direction,this.TileSeparationSteps);
...
//				Debug.Log ("Cell here: " + cell_here.location + " is " + cell_here.Type + " and next: " + cell_next.location + " is " + cell_next.Type);
