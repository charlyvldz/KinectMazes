...
public class TileMapInterpreter : MapInterpreter<TileMapInterpreterBehaviour> {
	...
	public void Fake3DEffect(VirtualMap map, VirtualMap[] maps, int storey){		
		...
		for (int i = 0; i < map.Width; i++){
			for (int j = 0; j <  map.Height; j++){
				map.cells[i,j] = output_cells[i,j];
			}
		}
	}
	protected void ConvertFake3DEffect(VirtualMap map, VirtualCell conversion_cell){
		CellLocation below_loc = map.GetNeighbourCell (conversion_cell.location, VirtualMap.DirectionType.South);
...
//					}
	...
	private void PerformTilemapConversion(VirtualMap map, VirtualMap[] maps, int storey){
		...
		for (int i = 0; i < map.Width; i++){
			for (int j = 0; j <  map.Height; j++){
				map.cells[i,j] = output_cells[i,j];
//				Debug.Log (map.cells[i,j].location + ": " + map.cells[i,j].Type);
			}
		}
	}
...
//					if (renderWall) ConvertFixedWallOrientation(map,conversion_cell);
				...
				else if (conversion_cell.IsNone()){// || conversion_cell.IsColumn()){
					// 'Ninguno' cells are usually converted to columns
					...
					// May be outside of the map
					if (map.IsRemovable(conversion_cell.location,behaviour.drawWallCorners) && !isPerimeter) ConvertRock(map,conversion_cell);
					else if (!isColumn) {
						if (CheckRoom(map,conversion_cell.location)){
							// An inside-room tile may be transformed into a floor
	...
	protected void ConvertTileIntersections(VirtualMap map, VirtualCell conversion_cell){
		...
		if(conversion_cell.IsEmpty()){			
			...
			// 'Ninguno' cells will remain as such, since they will be converted to columns later, if needed.
		}
	}
