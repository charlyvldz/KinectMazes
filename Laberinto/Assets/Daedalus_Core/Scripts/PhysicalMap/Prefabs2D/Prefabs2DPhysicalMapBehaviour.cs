...
public class Prefabs2DPhysicalMapBehaviour : PhysicalMapBehaviour<GameObject,GameObjectChoice> {
	...
	public float MeasureSize(VirtualCell.CellType type){
		...
		DebugUtils.Assert(false,"Cannot Measure Tile Size For Prefab " + prefab.name);
		return 0;
	}
	...
	override public bool CheckDefaults(){		
		...
		if (interpreterBehaviour is TileMapInterpreterBehaviour){
			...
			corridorWallOVariations = CheckDefault("Daedalus_Resources/2D/Prefabs2D/DefaultCorridorWallO",corridorWallOVariations);
			corridorWallUVariations = CheckDefault("Daedalus_Resources/2D/Prefabs2D/DefaultCorridorWallU",corridorWallUVariations);
			corridorWallIVariations = CheckDefault("Daedalus_Resources/2D/Prefabs2D/DefaultCorridorWallI",corridorWallIVariations);
			corridorWallLVariations = CheckDefault("Daedalus_Resources/2D/Prefabs2D/DefaultCorridorWallL",corridorWallLVariations);
			corridorWallTVariations = CheckDefault("Daedalus_Resources/2D/Prefabs2D/DefaultCorridorWallT",corridorWallTVariations);
			corridorWallXVariations = CheckDefault("Daedalus_Resources/2D/Prefabs2D/DefaultCorridorWallX",corridorWallXVariations);
			
			roomWallOVariations = CheckDefault("Daedalus_Resources/2D/Prefabs2D/DefaultRoomWallO",roomWallOVariations);
			roomWallUVariations = CheckDefault("Daedalus_Resources/2D/Prefabs2D/DefaultRoomWallU",roomWallUVariations);
			roomWallIVariations = CheckDefault("Daedalus_Resources/2D/Prefabs2D/DefaultRoomWallI",roomWallIVariations);
			roomWallLVariations = CheckDefault("Daedalus_Resources/2D/Prefabs2D/DefaultRoomWallL",roomWallLVariations);
			roomWallTVariations = CheckDefault("Daedalus_Resources/2D/Prefabs2D/DefaultRoomWallT",roomWallTVariations);
			roomWallXVariations = CheckDefault("Daedalus_Resources/2D/Prefabs2D/DefaultRoomWallX",roomWallXVariations);
