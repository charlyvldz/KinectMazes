...
//		roomDoorVariations = CheckDefault("DefaultRoomDoorTexture",roomDoorVariations);
		...
		corridorWallOVariations = CheckDefault("DefaultCorridorWallOTexture",corridorWallOVariations);
		...
		roomWallOVariations = CheckDefault("DefaultRoomWallOTexture",roomWallOVariations);
...
//		Debug.Log (defaultName);
		DebugUtils.Assert(loadedSpritesDict.ContainsKey(defaultName),"Cannot find default texture asset " + defaultName);
		return loadedSpritesDict[defaultName];
	}


	override protected SpriteChoice[] GetVariations(VirtualCell.CellType cell_type){
