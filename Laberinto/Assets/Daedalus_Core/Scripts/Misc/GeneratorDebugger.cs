...
public class GeneratorDebugger {
	...
	private List<GameObject> SelectInternal(PhysicalMap physicalMap, SelectionObjectType type){
		...
		DebugUtils.Assert(physicalMap.gameObjectLists != null, "Physical map's game object lists are null!");
	...
	public List<GameObject> SelectObjectsByZone(int zone_number){
		...
		DebugUtils.Assert(physicalMap.zones != null, "Physical map's zone lists are null!");
		DebugUtils.Assert(zone_number >= 0, "Zone number must be >= 0!"); 
		DebugUtils.Assert(zone_number <= physicalMap.zones.Count-1, "Zone number too high!"); 
