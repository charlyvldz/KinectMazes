...
public class PointManController : MonoBehaviour 
{
	...
	void Start () 
	{
		...
		// debug text
		//debugText = GameObject.Find("TextodeCalibracion");
	...
	{
		// get 1st player
		uint playerID = KinectManager.AdministradorKinect != null ? KinectManager.AdministradorKinect.GetPlayer1ID() : 0;
		...
		Vector3 posPointMan = KinectManager.AdministradorKinect.GetUserPosition(playerID);
		...
		for(int i = 0; i < jointsCount; i++) 
		{
			if(_bones[i] != null)
			{
				if(KinectManager.AdministradorKinect.IsJointTracked(playerID, i))
				{
					_bones[i].gameObject.SetActive(true);
					
					int joint = MirroredMovement ? KinectWrapper.GetSkeletonMirroredJoint(i): i;
					Vector3 posJoint = KinectManager.AdministradorKinect.GetJointPosition(playerID, joint);
					posJoint.z = !MirroredMovement ? -posJoint.z : posJoint.z;
					Quaternion rotJoint = KinectManager.AdministradorKinect.GetJointOrientation(playerID, joint, !MirroredMovement);
